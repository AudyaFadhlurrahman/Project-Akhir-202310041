...
export default function FormSignin() {
  ...
  const handleContinue = () => {
    // Menyimpan riwayat login ke local storage
    const loginHistory = JSON.parse(localStorage.getItem("loginHistory")) || {};
    const newLogin = { email: profile.email, timestamp: Date.now() };
    loginHistory[newLogin.timestamp] = newLogin;
  ...
  const handleSubmit = (e) => {
    ...
    if (profile.email && profile.password) {
      ...
      openModal({ header: "Information", message: <p className="text-danger">Please fill up the form correctly</p> });
    }
    ...
    <div className="py-10">
      <div className="card d-flex flex-column flex-center bg-white">
        ...
        <div className="card-body shadow-lg w-lg-500px">
          <form className="form-profile" method="post" onSubmit={handleSubmit} id="form-profile">
